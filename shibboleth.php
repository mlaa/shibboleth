...
function shibboleth_authenticate($user, $username, $password) {
	if ( shibboleth_session_active() ) {
		...
		// keep initiator url on the mapped domain by matching the redirect_to url
		// otherwise the session starts on a domain different from where user is trying to log in
		$redirect_to = ( function() {
			if ( isset( $_REQUEST['redirect_to'] ) ) {
				$redirect_to = $_REQUEST['redirect_to'];
			} else if ( isset( $_COOKIE['redirect_to'] ) ) {
...
function shibboleth_logout() {
	...
	if ( isset( $parsed_logout_url['host'] ) && $parsed_logout_url['host'] !== COOKIE_DOMAIN ) {
		...
		if (
			isset( $parsed_query['return'] ) &&
			false !== strpos( $parsed_query['return'], 'logged-out' ) &&
			false === strpos( $parsed_logout_url['host'], COOKIE_DOMAIN )
		) {
			$parsed_query['return'] = '/';
			$logout_url =
